  ...
  const testAuth = async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch(
        'https://groceries-api-m1sq.onrender.com/api/v1/orders',
      )
  ...
  return (
    <div className="p-4 bg-gray-100 rounded-lg">
      ...
      <div className="space-y-2 text-sm mb-4">
        ...
        <p>
          <strong>Token from store:</strong> {token ? 'Present' : 'None'}
        </p>
        <p>
          <strong>Token from getToken():</strong>{' '}
          {currentToken ? 'Present' : 'None'}
        </p>
        <p>
          <strong>Token value:</strong> {token || 'No token'}
        </p>
      </div>
